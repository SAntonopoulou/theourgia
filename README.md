...
| | |
|---|---|
| **Latest commit** | `v1-022` — **prod celery incident fully closed** (worker module path + restic in image + creds passthrough + beat schedule path + asyncpg cross-loop fix with live regression test; first real prod backup verified in R2) · **Wave 2**: memorial auto-trigger + posthumous release + Shamir key-share (alembic 0079) · Beings ledger routes mounted from the H03 designs (celery never ran in prod: wrong -A module since first deploy · restic now in image · backup creds passthrough · worker consumes backups queue · first real prod backup lands with this deploy) — on top of **the 2026-07-16 v1.0 close-out wave** (18 batches so far): MBF bundle format per ADR-0011 (export → sign → preview → import verified live) · local Whisper transcription (dual opt-in) · Islamic/Coptic/Mayan/French-Republican calendars · closed-tradition substrate + first-class entry tags · crisis-nudge backend · Stream/Mux video providers · bind-rune designer + Northumbrian bundle · Postmark/SES/Mailgun delivery · Journal FTS search wired · pendulum/scrying persistence + trance mode · Helm chart · installer + monitoring + 20 runbooks · GDPR doc set · 17 user docs · fresh-install fixes (pgvector image; migration chain now replays 0001→0078 from zero, verified on real Postgres) |
| **Production** | **🟢 LIVE at https://theourgia.com** (deployed 2026-06-28; 8 prod containers, isolated compose project) |
| **vitest** | **3158** shared + **56** admin + **3288** backend passing · admin tsc clean · shared tsc clean · **zero `as any` casts · zero `@ts-ignore`** |
| **backend** | **3288 passing** · alembic head **0078** — entry tags + tradition_tags; installed_bundle + sandbox bundle columns; webauthn RLS re-assert; 0066 replay guard; wellbeing + audio + calendars settings; traditions/bundles/wellbeing/audio routers on `CurrentUser` |
