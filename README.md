...
| | |
|---|---|
| **Latest commit** | `b108-2ia` (**Print-quality book PDF export** · Tier plan #19 · new `core/publishing/book_pdf.py` renders a Publication + PublicationChapter rows to a real trade-paperback PDF (6×9 in trim · asymmetric gutter margins · title / copyright / TOC front matter · chapters open on recto pages · running headers · widow / orphan control · justified body with hyphenation · Tiptap visitor pattern parallel to the Obsidian exporter) · `GET /api/v1/publications/{id}/book-pdf` owner-only · admin route `/publications/:id/print-preview` wires shared `PrintPreviewSurface.onExport` to Blob download · `ApiClient.requestBlob` primitive for binary downloads) |
| **Production** | **🟢 LIVE at https://theourgia.com** (deployed 2026-06-28; 8 prod containers, isolated compose project) |
| **vitest** | **3039** shared + **40** admin route-mount smoke + **2974** backend passing · admin tsc clean · shared tsc clean · **zero `as any` casts · zero `@ts-ignore`** |
