...
4. **Hosted SaaS:** Likely future feature once federation matures. **The code stays AGPL-3.0 forever even when offered as a paid hosted service** — no proprietary forks, no closed-source tiers, no "community vs. enterprise" editions. Revenue model is hosting fees plus a small profit margin (offsetting infrastructure + operational costs); the project's primary maintainer has other income and is not building this to be a revenue engine. Competitive moat is operational quality — managed support, curated official bundles, reliability, community trust — never withheld code. Anyone may run a competing hosted Theourgia using the same source; this is by design, because the project's mission is community infrastructure, not market capture. **Copyleft is the only way.**
