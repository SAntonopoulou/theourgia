...
| | |
|---|---|
| **Latest commit** | `v1-028` — **Wave 3**: federation transport wired end-to-end (inbox processing, AP signature verification, Follow→Accept, Create-on-publish, NodeInfo 2.0 without metrics, peers CRUD + capability tokens, alembic 0080) · Mode A vault-key rotation with convergent crash-safe sweep (alembic 0081) · entry version history (snapshots, restore, sealed-purge guard) · after the 2026-07-20 prod incident closure (first-ever verified R2 backup + restore drill; docs/ops/INCIDENT-2026-07-20) and Waves 1-2 (28 batches: MBF + 7 content bundles, Whisper, 4 calendars, closed-tradition + tags, memorial completion, Beings routes, crisis nudge, Stream/Mux, bind-rune, email backends, FTS, divination persistence + trance, Helm, ops bundle, GDPR docs, 17 user docs, fresh-install fixes) |
| **Production** | **🟢 LIVE at https://theourgia.com** (deployed 2026-06-28; 8 prod containers, isolated compose project) |
| **vitest** | **3190** shared + **110** admin + **3440** backend passing · admin tsc clean · shared tsc clean · **zero `as any` casts · zero `@ts-ignore`** |
