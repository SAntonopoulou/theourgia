...
comments mark their source** (H08 rule 24) — handle in
`--font-mono` `--remote` + verbatim `‡ from {instance}` chip.
Local comments render the same layout otherwise — federation
never gets ghosted into a "special quote box." Three sections:
**Approved** (default open) · **Pending moderation**
(owner-only) · **Hidden** (owner-only). Owner-only sections
wear a verbatim `Only you can see this` lock badge. **Hide /
Unhide flips with section** (verb names the resulting state).
Flag is hidden inside the Hidden section (flagging an already-
hidden comment is no-op). No like / repost / engagement
counters anywhere — only the per-section count. 18 new tests;
shared 2620 → **2638**. **20/21 done · one surface remaining.**

**H08 surface 19/21 — WebFinger Verification** (2026-06-27 ·
`/verify`). Three-step lookup that ties a Fediverse handle to
the vault's actor + signing key. **Failures never blame**
(H08 rule 25) — fail subtitle is verbatim: **"The handle did
not resolve to your vault. This is a configuration issue, not
an error on your part."** Failure card uses `--warn-soft` NEVER
`--danger` (a misconfiguration is fixable, not catastrophic).
Pass card surfaces the **full SHA256 key fingerprint** in
`--peer-ok` `--font-mono` so the user can compare against an
out-of-band channel. Run-check button refuses on empty handle;
loading state shows inline spinner + verbatim "Querying
WebFinger…". 11 new tests; shared 2609 → **2620**. **19/21
done.**

**H08 surface 18/21 — Remote Content Embed primitive** (2026-06-27).
Tiptap-node-shaped card for displaying federated Mastodon /
Pleroma / Fediverse posts inline in an entry. **Three states**:
**resolvable** (full post · `--remote` border + chip ·
`‡ from {instance}` chip · `View original →` link in `--remote`)
· **loading** (shimmer skeleton · `aria-busy=true` ·
`role=status`) · **unresolvable** (dashed border on
`--bg-sunk` · verbatim **"Original post no longer available"** ·
**citation preserved** in `--font-mono` — `{handle} ·
‡ from {instance}` survives the post's disappearance). Author
handle in every info-bearing state is `--font-mono` `--remote`
so readers see the wire form. The H08 brief is explicit:
**"remote words are never quietly absorbed into your own."** 9
new tests; shared 2600 → **2609**. **18/21 done.**

**H08 surface 17/21 — Followers Pane** (2026-06-27 · `/followers`).
Two-tab pane: Followers (default) · Pending approvals (with
count chip in `--warn-soft` when non-empty). **No engagement
metrics** (H08 rule 18 · anti-gamification): the verbatim
helper above the list reads **"Listed in the order they
followed — newest first. Theourgia keeps no engagement metrics
beyond this count."** No likes, no reposts, no view counts, no
ranking. **Consent-first follows** (rule 19): pending tab
explicitly Approve (`--accent`) / Decline (`--warn-soft`, NEVER
`--danger` — refusing a follow is protective, not punitive).
Verbatim pending callout: **"You approve follows manually.
These accounts asked to follow your public actor; approving
lets them see your public posts in their timeline."** Empty
state: **"No requests waiting."** WebFinger handle in
`--font-mono` `--remote`. 19 new tests; shared 2581 → **2600**.
**17/21 done.**

**H08 surface 16/21 — ActivityPub Settings** (2026-06-27 ·
`/settings/activitypub`). **Per-network opt-in** (rule 28).
Master switch is OFF by default; the body dims to `opacity .42
· pointer-events:none` until the user explicitly enables. First
activation requires a `--danger` alertdialog (rule 2 ·
`--danger` is RESERVED for Visibility-becoming-Public-equivalent
moments — this is that step). Verbatim title: **"Open your
public actor to the wider web"** · verbatim sub: **"This is the
one irreversible-feeling step in Theourgia."** ON→OFF is single
tap; the brief intentionally gates only the first activation.
Four bands: Public profile (WebFinger handle in `--font-mono`
chip) · Follower approval radio · Object-type mapping (Essays →
Article · Notes → Note · Rituals → Event · Publications →
Article) · Outbound activities (Create / Update default ON;
**Delete defaults OFF** with verbatim cache caveat: "remote
caches keep copies regardless"). 20 new tests; shared 2561 →
**2581**. **16/21 done — Cluster B open.**

**H08 Cluster A COMPLETE — 15/21** (2026-06-27).

**H08 surface 15/21 — Push Content to Hub modal** (2026-06-27).
Federated push moment with two states. **Network entry** branch:
hub checkboxes ("you're {role}") with per-hub curation chip —
auto-curating hubs render `--warn-soft` ("This hub
auto-curates") so the user knows their submission publishes
without human review; reviewing hubs render `--peer-ok-soft`
("This hub reviews submissions"). Push CTA uses `--warn-soft`
(consequential edit) NEVER `--accent`/`--danger`. **Sealed
entry** branch: `--seal-soft` callout with verbatim **"Sealed
entries cannot be pushed."** + **"Sealed content never
federates."**; Push CTA disabled, refuses on its own even if
the consumer passes initial selection. Cache-persistence is
disclosed verbatim under the picker: **"Content already
mirrored may persist in caches."** No "push to all hubs"
shortcut. 20 new tests; shared 2541 → **2561**. **15/21 done —
Cluster A is closed; Cluster B (ActivityPub, 6 surfaces) is
next.**
...
central SSO authority** — the user grants ONE assertion to ONE
hub, no broker, no Theourgia ID). Three **mandatory sections**
in fixed order: `What the hub wants to verify` (identity DID
in `--font-mono`) · `What the hub will receive` (minimal claims
line) · `What this assertion authorizes` (scope + 24h expiry +
revoke path). `‡ from {instance}` chip uses `--remote` chrome.
Verbatim `--warn-soft` callout: **"This is NOT a login. Your
home instance never sees the hub's pages directly — only this
consent moment."** Esc + scrim click → **decline** (never
approve — no implicit consent). The modal collects NO
credentials. 17 new tests; shared 2506 → **2523**. **13/21
done.**

**H08 surface 12/21 — Roles & Permissions Editor** (2026-06-27 ·
`/hubs/:hubId/admin/roles`). 11-capability × 5-role matrix
(admin · officer · moderator · member · observer). Capabilities
are **explicit** — there is no hidden "implies" relationship;
the wire is rule-based. **Save changes** stages the draft;
**Save + apply** (`--warn-soft` chrome, never `--danger`) is the
edge that propagates immediately so the next denied action
surfaces the verbatim banner: **"You cannot do {action} because
you lack permission {permission}."** Two-slot interpolation,
no softening pre-amble. `+ Add custom role` defaults to
least-privilege (zero caps). `Preview as role` is a read-only
lens — never mutates the matrix. 20 new tests; shared 2486 →
**2506**. **12/21 done.**
