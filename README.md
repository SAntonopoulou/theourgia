...
| | |
|---|---|
| **Latest commit** | `v1-021` — **prod celery incident fix** (celery never ran in prod: wrong -A module since first deploy · restic now in image · backup creds passthrough · worker consumes backups queue · first real prod backup lands with this deploy) — on top of **the 2026-07-16 v1.0 close-out wave** (18 batches so far): MBF bundle format per ADR-0011 (export → sign → preview → import verified live) · local Whisper transcription (dual opt-in) · Islamic/Coptic/Mayan/French-Republican calendars · closed-tradition substrate + first-class entry tags · crisis-nudge backend · Stream/Mux video providers · bind-rune designer + Northumbrian bundle · Postmark/SES/Mailgun delivery · Journal FTS search wired · pendulum/scrying persistence + trance mode · Helm chart · installer + monitoring + 20 runbooks · GDPR doc set · 17 user docs · fresh-install fixes (pgvector image; migration chain now replays 0001→0078 from zero, verified on real Postgres) |
