...
> **🔧 Phase 10 Publishing & Monetization backend SHIPPED.** B126-B131 closed 2026-06-26: publication lifecycle (4-state · sealed-embed rejection at publish + republish) · Stripe Connect substrate (Protocol-isolated SDK; 0% application fee CI invariant; refunds via portal hand-off — NO `/refund` POST endpoint anywhere in any router) · single-use download tokens (32-byte url-safe, HMAC-SHA256, 30-day TTL, 5-download limit) · subscription tiers with IMMUTABLE amount (Stripe prices don't change in place) · double-opt-in subscribers (PENDING_CONFIRMATION default; FAILED_PAYMENT = `--warn` never `--danger`; sticky unsubscribe) · newsletter 5-state lifecycle (once-sent immutability; `confirmation_required: true` on send-now per the H07 `--warn-soft` confirm modal contract; per-recipient unsubscribe URL in every render; Tiptap → HTML/plaintext renderer with `html.escape` XSS guard) · public reader with structural paywall (`paywall_kind: "none" | "purchase" | "subscribe"` + URLs only; ReaderResponse schema actively REJECTS countdown timer / "limited time" / view_count / trending fields — defensive CI test enumerates banned field names) · per-vault public page (popular-sort opt-in defaults FALSE; anti-gamification — no view_count / trending / subscriber_count on public surfaces) · unversioned RSS 2.0 / Atom 1.0 / JSON Feed 1.1 serialisers at `/vaults/{id}/feed.{rss,atom,json}` (feeds mounted at app-level NOT `/api/v1` so subscribers' URLs stay stable; every item carries per-publication license slug AND AGPLv3 site-wide credit; XSS-safe escaping in all three formats). Sealed publications NEVER public — defence in depth on top of B126 publish-time + B127 checkout-time checks. Withdrawn 404s. 2073 backend tests passing; alembic 0047 → 0051.

> **🔧 Phase 09 Synchronicity & Analytics backend [solo subset] SHIPPED.** B120-B125 closed 2026-06-26: synchronicity table + autotag (location-precision floor enforced server-side) · QUERY_BUILDER study kind + saved-query DSL · executor with sealed exclusion guard + `sealed_excluded_count` · timeseries / heatmap / correlation / today aggregates (every response carries sample_size + small_sample flag) · weekly digest builder (banned-phrase regex blocks modal/oracular headlines). 1899 backend tests passing; alembic 0042 → 0047. Network-aggregate path (DP noise, anonymized cross-vault, federation) deferred to Phase 12+. **All 10 H06 surfaces shipped** (Gematria Calculator · Cross-Journal Search · Per-Study Page · Voces Library · Studies Index · Transliteration Utility · Analytics Dashboard · Query Builder · Synchronicity Log · Quick-Capture).
>
> **🔧 Phase 08 Linguistic Tools backend SHIPPED.** B110-B114 closed 2026-06-26: cipher catalog (13 PD bundled fixtures across six languages) · gematria index + cross-journal search (3 match modes · sealed entries never indexed · CSV export) · studies + frozen snapshots (immutable queries after first save · re-run creates new snapshots) · 8 PD transliteration scheme reference tables · voce per-vault state (private notes + per-vault hide). Backend now 1753 tests passing; Alembic chain 0037 → 0042.
>
> **🔧 Phase 07 backend SHIPPED.** B103-B107 closed: five domains live across Alembic migrations 0033 → 0037 (1625 backend tests passing). B108-2 frontend wiring is largely complete — five of six Workshop surfaces persist via real API calls, sealed talismans round-trip through PBKDF2 + AES-GCM client-side encryption. Tool Registry create form is the only outstanding piece (designer follow-up). **H06 designer bundle for Phases 08-09 received 2026-06-25** — 10 surfaces queued for the next sprint, blocked on B108-2e completion.
...
| 07 | [Workshop](plan/07-workshop.md) | ✅ backend done · frontend wired (B108-2e pending) | Sigil generator (11 modes incl. Spare · Kamea · Hashed-vector · Harmonograph · sandboxed Parametric formula), magic squares (7 Agrippa planetary fixtures + custom builder), talisman designer (composite with name-rings + kamea embeds + Mode B sealed encryption — PBKDF2 + AES-GCM end-to-end), magical circle (rings + compass + centre + 5 PD presets), tool registry (14 kinds + altars + consecrate sub-resource), voces magicae (32-entry PD bundled corpus from PGM + Sefer Yetzirah + Lemegeton + Heptameron + Sanskrit). **Sprint summary**: H05 frontend (B89-B96) + B103-B107 backend (5 domains · alembic 0033-0037 · 152 tests) + B108-1/2a/2b/2c/2d frontend wiring. Tool Registry create form (B108-2e) queued — surface emits intent only; form composition not in scope of H05 handoff. |
| 08 | [Linguistic Tools](plan/08-linguistic-tools.md) | ✅ backend done · H06 frontend partial | Cipher catalog (13 PD bundled · superset of client-side seven, byte-for-byte parity) · gematria_index + cross-journal search (3 match modes · sealed entries never indexed) · studies + frozen snapshots · 8 PD transliteration schemes (Beta Code · ALA-LC · IAST · Harvard-Kyoto · SBL Hebrew · ISO 233 · DIN 31635 · SBL Coptic) · voce per-vault state. H06 surfaces 1/4 (Gematria Calculator) + 4/4 (Voces Library Browser) shipped; 2 + 3 + 5-10/10 queued (now unblocked). |
...
**Phase 08 backend (B110-B114): 5 new tables · 5 migrations · 4 routers · 128 tests · 13 PD bundled ciphers · 8 PD transliteration schemes.**
**Phase 09 backend solo subset (B120-B124): 4 new tables · 3 migrations · 3 routers · 146 tests · 10-category synchronicity log · DSL+executor with sealed-exclusion guard · banned-phrase regex on digest headlines.**
**Phase 10 backend (B126-B130): 7 new tables · 4 migrations (0048 → 0051) · 6 routers · 174 tests · 9-license picker · 0% application fee CI invariant · refunds via portal hand-off (no `/refund` POST endpoint anywhere) · structural paywall (closed Literal kind + URLs only) · unversioned RSS/Atom/JSON Feed with AGPLv3 credit · sealed publications NEVER public (defence in depth at publish + checkout + read).**
**Phase 11 backend (B132-B135): 5 new tables · 4 migrations (0051 → 0055) · 4 routers · 203 tests · polymorphic media_link · Protocol-isolated EXIF strip (NullExifStripper + PillowExifStripper) · 5 GB quota · 24h upload session TTL · precision FLOOR one-way ratchet (shared apply_precision_floor helper) · SealedDayMarker dataclass restricted to {date, count} · "{N} sealed entries today" iCal sealed-day collapse · unversioned `/ical/v1/{token}.ics` mounted at app-level.**
**H01-H03 sprint: 71 primitives across 22 modules. H04 sprint: 7 designed surfaces + Daily Practice Tracker · 7 headless divination engines · 6 new design tokens. H05 sprint: 6 Workshop surfaces + 6 SVG engines. H06 bundle received 2026-06-25 (10 surfaces · queued).**
**Phase 07 backend (B103-B107): 5 domain models · 8 migrations · 7 routers · 152 tests · 7 Agrippa planetary squares · 5 PD circle presets · 32 PD voces. Frontend wiring (B108-1/2a-d): live save for Sigil + MagicSquares + Talisman + Circle + Voces · Mode B vault crypto (PBKDF2 + AES-GCM, 11 round-trip / tamper / wrong-key tests).**
...
- [x] **DRM-free always** — optional watermarking with buyer email
...
- [~] **Reference plugins shipped**:
  - [~] Norse runes extended (Younger Futhark, Anglo-Saxon Futhorc, bind-rune designer) — b108-2ho shipped Younger Futhark Long Branch (16 runes, c. 800-1100 CE) · b108-2hp shipped Anglo-Saxon Futhorc (33 runes: 24 Elder base + 5 OE additions Ac/Æsc/Yr/Ior/Ear + 4 Northumbrian Cweorð/Calc/Stan/Gar; meanings from the *Anglo-Saxon Rune Poem* c. 8th-10th c.). Armanen (18 runes) + Northumbrian standalone bundle + bind-rune designer surface all follow in later batches.
  ...
  - [x] Matrix notification channel — b108-2hv: `core/notifications/channels/matrix.py` implements the `NotificationChannel` protocol · POSTs `m.notice` messages (not `m.text`) to `PUT /_matrix/client/v3/rooms/{roomId}/send/m.room.message/{txnId}` · bearer-token auth · random 32-hex txn_id per send (homeserver dedupes on collision) · transport injected as a Protocol so tests don't need a live homeserver · action_url + action_label appended to body · one attempt + clean `NotificationDeliveryError` on failure (retries live in the notification service, not the channel — regression-guarded). `DeliveryChannel.MATRIX` enum value added. Rejects config with empty homeserver/token, missing scheme, or room alias (must be internal `!id:server`).
