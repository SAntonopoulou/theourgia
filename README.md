...
| | |
|---|---|
| **Latest commit** | `b108-2hz` (**Hebrew month renders "Tammuz" not "month 4"** — bug Sophia caught after b108-2hy shipped the auto-stamp box · `_serialise_calendar_date` now pulls `month_name` from the Hebrew calendar's `raw` dict + surfaces the pre-rendered `long`/`short`/`numeric` strings · frontend `HEBREW_MONTH_NAMES` arrays fixed from Tishri-starting to **Nisan-starting** in both `formatSnapshots.ts` and `blog-read.astro` · both frontends prefer the backend's pre-rendered `long` string when present · regression guard: `NEVER emits "month N"` for Hebrew) |
| **Production** | **🟢 LIVE at https://theourgia.com** (deployed 2026-06-28; 8 prod containers, isolated compose project) |
| **vitest** | **3039** shared + **39** admin route-mount smoke + **2949** backend passing · admin tsc clean · shared tsc clean · **zero `as any` casts · zero `@ts-ignore`** |
| **backend** | **2949 passing** · alembic head **0075** — entry.published_at column; tea_leaf_reading; memorial_config; family-tree kinship enum + ancestor_profile column; recipe + pilgrimage_route + comment + content_format; every write endpoint + owned read on `CurrentUser` |
