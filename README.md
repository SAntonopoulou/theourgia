...
| | |
|---|---|
| **Latest commit** | `v1-038` — **five-feature wave** (agent-daemon vault-side MCP with SQL-level sealed exclusion + runs persistence + 6 agent definitions · Phase 14 registry release hosting + signed installs + SSO bridge + startup plugin loader · cross-instance group rituals + egregore + DP hub aggregates · seal reconciliation with a real one-way seal ceremony) — merged from concurrent batches, chain replays 0001→0085 from zero, all suites green · earlier: 2026-07-20 prod incident closed (first R2 backup + restore drill) · vault provisioning blocker fixed + backfilled on prod · twin-instance federation proven live (signed handshake + replay rejection)
| **Production** | **🟢 LIVE at https://theourgia.com** (deployed 2026-06-28; 8 prod containers, isolated compose project) |
| **vitest** | **3201** shared + **123** admin + **3595** backend passing · admin tsc clean · shared tsc clean · **zero `as any` casts · zero `@ts-ignore`** |
