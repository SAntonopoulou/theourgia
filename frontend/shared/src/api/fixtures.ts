...
export function defaultFixtures(path: string, init?: RequestInit): unknown {
  ...
  // /api/v1/astro/planetary-hours — a deterministic day: sunrise 06:00,
  // sunset 20:00 (long day hours), Saturday (the day of Saturn), current
  // hour index 8. Times are built around "now" so the ring's needle and
  // countdown land mid-day in any session.
  if (bare === "/api/v1/astro/planetary-hours" && method === "GET") {
    ...
    const now = Date.now();
    const sunrise = now - 8.5 * 70 * 60_000;
    const dayLen = 70 * 60_000;
    const nightLen = 50 * 60_000;
    const hours = Array.from({ length: 24 }, (_, i) => {
      const isDay = i < 12;
      const start = isDay ? sunrise + i * dayLen : sunrise + 12 * dayLen + (i - 12) * nightLen;
      const len = isDay ? dayLen : nightLen;
      const ruler = order[(0 + i) % 7] as string;
      return {
        index: i,
        ruler,
        glyph: glyphs[ruler] ?? "·",
        start: new Date(start).toISOString(),
        end: new Date(start + len).toISOString(),
        is_day: isDay,
      };
    });
    return {
  ...
  if (bare === "/api/v1/astro/moon-course" && method === "GET") {
    return {
      ...
      next_sign_ingress: new Date(Date.now() + 9 * 3_600_000).toISOString(),
      void_thirty_degrees: false,
      void_sign_exit: true,
      rule: "thirtyDegrees",
      void: false,
      attribution: "Swiss Ephemeris (mock fixture)",
  ...
  // /api/v1/astro/chart/doctrine — the server-derived traditional reading
  // for the mock chart below. NOT hand-derived: generated by running the
  // backend's _judge_planet/all_lots over the same longitudes (Asc 0°,
  // Sun 12.5° → below the horizon → nocturnal), so the fixture is canon.
  if (bare === "/api/v1/astro/chart/doctrine" && method === "GET") {
    return {
      sect: {
        sect: "nocturnal",
        light: "moon",
      ...
      lots: [
        { id: "fortune", label: "Fortune", longitude: 280.5 },
        { id: "spirit", label: "Spirit", longitude: 79.5 },
        { id: "eros", label: "Eros", longitude: 41.5 },
        { id: "necessity", label: "Necessity", longitude: 97.5 },
        { id: "courage", label: "Courage", longitude: 224.5 },
        { id: "victory", label: "Victory", longitude: 244.5 },
        { id: "nemesis", label: "Nemesis", longitude: 4.5 },
      ],
      dignities: [
        {
          body_id: "sun",
        ...
        },
        {
          ...
          sign: "Cancer",
          domicile_lord: "moon",
          exaltation_lord: "jupiter",
          triplicity_lord: "mars",
          bound_lord: "mars",
          decan_lord: "venus",
          held: ["domicile"],
          debilities: [],
        ...
        },
        {
          ...
          sign: "Taurus",
          domicile_lord: "venus",
          exaltation_lord: "moon",
          triplicity_lord: "moon",
          bound_lord: "venus",
          decan_lord: "mercury",
          held: ["domicile", "bound"],
          debilities: [],
        ...
        },
        {
          ...
          sign: "Libra",
          domicile_lord: "venus",
          exaltation_lord: "saturn",
          triplicity_lord: "mercury",
          bound_lord: "jupiter",
          decan_lord: "saturn",
          held: ["bound"],
          debilities: [],
        ...
        },
        {
          body_id: "saturn",
          sign: "Capricorn",
          domicile_lord: "saturn",
          exaltation_lord: "mars",
          triplicity_lord: "moon",
          bound_lord: "venus",
          decan_lord: "mars",
          held: ["domicile"],
  ...
  // /api/v1/users/me/settings/astro-doctrine — the ledger defaults; PUT echoes.
  if (bare === "/api/v1/users/me/settings/astro-doctrine") {
