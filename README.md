...
| | |
|---|---|
| **Latest commit** | `b108-2hg` (memorial mode / digital inheritance v1 · `memorial_config` table + alembic 0073 · `/api/v1/memorial/{config,check-in,trigger,reactivate}` endpoints with computed active/warning/pending/memorialized state · `MemorialModeSurface` primitive + admin `/memorial-mode` route with status card + configurable cadence + executor contact + public message + posthumous-publications opt-in · FEATURES §18 memorial mode + check-in mechanic `[ ]` → `[x]`; executor + posthumous `[~]` — key-share crypto + release job follow up) |
| **Production** | **🟢 LIVE at https://theourgia.com** (deployed 2026-06-28; 8 prod containers, isolated compose project) |
| **vitest** | **2987** shared + **38** admin route-mount smoke + **2757** backend passing · admin tsc clean · shared tsc clean · **zero `as any` casts · zero `@ts-ignore`** |
| **backend** | **2757 passing** · alembic head **0073** — memorial_config table; family-tree kinship enum + ancestor_profile column; recipe + pilgrimage_route + comment + content_format; every write endpoint + owned read on `CurrentUser` |
