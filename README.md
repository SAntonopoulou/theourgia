...
| | |
|---|---|
| **Latest commit** | `b108-2hx` (Video integration · new `videoEmbed` Tiptap block + `/video` slash command · YouTube URLs (canonical / short / embed / shorts / m.) or bare 11-char IDs all normalise via `extractYoutubeId()` · privacy-enhanced host `youtube-nocookie.com` · lazy-loaded iframe (no third-party requests until scrolled to) · captions_url (.vtt) + chapters textarea supporting `mm:ss`, `h:mm:ss`, and bare-seconds formats · chapters render as clickable timestamp buttons that seek via `startSeconds` · **never autoplays** by default (regression-guarded) · blog reader detail page renders the same iframe) |
| **Production** | **🟢 LIVE at https://theourgia.com** (deployed 2026-06-28; 8 prod containers, isolated compose project) |
| **vitest** | **3019** shared + **39** admin route-mount smoke + **2922** backend passing · admin tsc clean · shared tsc clean · **zero `as any` casts · zero `@ts-ignore`** |
