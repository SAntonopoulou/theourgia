...
describe("planTotals", () => {
  ...
  it("sums a repeating cycle (a breath the phone authored)", () => {
    const plan = JSON.stringify({
      segments: [
        { segment: "cycle", repeats: 4, steps: [{ kind: "sit", seconds: 4 }, { kind: "sit", seconds: 6 }] },
      ],
    });
...
describe("meditationPlansFromEntries", () => {
  const plan = (row: Record<string, unknown>, envelope: Partial<MeditationRecordEntry> = {}): MeditationRecordEntry => ({
    kind: "meditation",
    doc: { row },
    ...envelope,
  });
  ...
  it("reads sitting plans, computing their length", () => {
    const plans = meditationPlansFromEntries([
      plan({ id: "m1", name: "Morning sit", summary: "Silence", kind: "sitting", plan: buildSitPlanJson(15, true), createdAt: "2026-08-20T00:00:00Z" }),
    ]);
    expect(plans).toHaveLength(1);
    expect(plans[0]).toMatchObject({ id: "m1", name: "Morning sit", seconds: 900, bell: true });
  });

  it("leaves breath plans to the phone", () => {
    const plans = meditationPlansFromEntries([
      plan({ id: "b1", name: "Box breath", kind: "breath", plan: "{}" }),
      plan({ id: "m1", name: "Sit", kind: "sitting", plan: buildSitPlanJson(5, false) }),
    ]);
    expect(plans.map((p) => p.id)).toEqual(["m1"]);
  });

  it("drops deleted plans and orders newest-first", () => {
    const plans = meditationPlansFromEntries([
      plan({ id: "a", name: "Old", kind: "sitting", plan: "{}", createdAt: "2026-08-01T00:00:00Z" }),
      plan({ id: "b", name: "New", kind: "sitting", plan: "{}", createdAt: "2026-08-20T00:00:00Z" }),
